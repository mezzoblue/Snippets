<?
	...
	function exportDB($connection, $name, $file, $local = "", $remote = "", $tables = '*') {
	    ...
	    for ($i = 0; $i < $num_fields; $i++) {
	      while($row = mysql_fetch_row($result)) {
	        ...
	        for($j = 0; $j < $num_fields; $j++) {
			  $row[$j] = swapHosts($row[$j], $local, $remote);
	...
	// swap out local host for remote when exporting the db	
	function swapHosts($str, $local, $remote) {
		if ($local && $remote) {
			// use the quicker strpos to find a hit, and rule out position 0 false negatives
			if (strpos($str, $local) !== false) {
				// if found, run a replace on the entire thing since there can be multiple instances
				return (str_replace($local, $remote, $str));
			} else {
				return($str);
			}
		}
